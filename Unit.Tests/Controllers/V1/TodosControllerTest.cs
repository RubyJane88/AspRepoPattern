...
using DemoSeven.WebApi.Controllers.V1;
using DemoSeven.WebApi.Data;
...
namespace Unit.Tests.Controllers.V1
{
    public class TodosControllerTest
    {
        ...
        public async Task GetTodosTest()
        {
            ...
            _mockRepo.Setup(repository => repository.GetAllAsync()).Returns(Task.FromResult(mockTodoDtos));
            
            //act
            var result = await _controller.GetTodos();
            var response = (OkObjectResult)result;
            var todos = response.Value as IEnumerable<TodoDto>;  //Cast the value to IEnumerable<TodoDto>
            
            //assert
            Assert.IsAssignableFrom<IActionResult>(result);
            Assert.NotNull(todos);
            Assert.Equal(200, response.StatusCode);
            Assert.Equal(4, todos.Count()); //else we cannot do the "todos.Count" 
        ...
        public async Task GetTodoByIdTest(string validGuid, string invalidGuid)
        {
            ...
            _mockRepo.Setup(repository => repository.GetByIdAsync(validItemGuid)).Returns(Task.FromResult(mockTodoDto));
            ...
            var mockInvalidTodoDto = MockData.GetAllTodos().FirstOrDefault(t => t.Id == invalidItemGuid); //test fails when I use just "First"
